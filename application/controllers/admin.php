...
$this->showAddOrg($data);
	...
	private function ResetOrganizationPassword($organization,$result)
	{
		$this->load->model('admin_model');
		$org_res=$result['org_res'];//organization info
		$user_res=$result['user_res'];//org user info
	...
	private function OrganizationProfile($result)
	{
		$this->load->model('admin_model');
		$org_res=$result['org_res'];//organization info
		$user_res=$result['user_res'];//org user info
 			...
 			if ($action =='new' && $secondaction == ''){
				...
				if(!$result){
					...
					if($secondaction != '' && $secondaction =='password-reset'){
						$this->ResetOrganizationPassword($action,$result);
					}else{
